...
SELECT TOP(3) CUST_NAME,CUST_CITY,CUST_COUNTRY,PAYMENT_AMT FROM CUSTOMER
...
SELECT CUST_CITY,AVG(OPENING_AMT) AS average_amount_opened  FROM CUSTOMER
GROUP BY CUST_CITY
ORDER BY average_amount_opened DESC

-- Info on the city with that receives payment the most
SELECT TOP(3) CUST_CITY,SUM(RECEIVE_AMT) AS SUM_AMOUNT_RECEIVED FROM CUSTOMER
GROUP BY CUST_CITY
ORDER BY SUM_AMOUNT_RECEIVED DESC

-- what working area has the most customer base
SELECT WORKING_AREA, COUNT(WORKING_AREA) AS number_of_mostCustomerbase FROM CUSTOMER
GROUP BY WORKING_AREA
ORDER BY number_of_mostCustomerbase DESC

-- chose what city to avoid due to rising debt
SELECT TOP(5) CUST_CITY, SUM(OUTSTANDING_AMT) AS rising_debt FROM CUSTOMER
GROUP BY CUST_CITY
ORDER BY rising_debt DESC
...
SELECT CUST_COUNTRY, COUNT(*) AS num_of_customer  ,SUM((OPENING_AMT + RECEIVE_AMT + PAYMENT_AMT) - OUTSTANDING_AMT ) AS amount_remaining   FROM CUSTOMER
GROUP BY CUST_COUNTRY
ORDER BY amount_remaining DESC 
...
---BETWEEN Helps to search for values within a range
---LIKE used to search for patterns in a text
--- IN specifies multiple values for a column

---BETWEEN Clause
SELECT CUST_NAME,WORKING_AREA,OPENING_AMT FROM CUSTOMER
WHERE OPENING_AMT BETWEEN 5000 AND 7000
